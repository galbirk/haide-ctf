...
{{- define "tcpHelper" -}}
{{- range $i, $e := until (int .Values.global.numberOfTeams) -}}
  "80{{ $i }}": "{{ $.Release.Namespace }}/{{ $.Values.global.TwentyFortyEight.name }}-svc-{{ $i }}:80"
{{ end }}
{{- range $i, $e := until (int .Values.global.numberOfTeams) -}}
  "22{{ $i }}": "{{ $.Release.Namespace }}/{{ $.Values.global.TwentyFortyEight.name }}-svc-{{ $i }}:22"
{{ end }}
{{- range $i, $e := until (int .Values.global.numberOfTeams) -}}
